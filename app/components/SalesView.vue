<template>
  <div class="min-h-screen bg-slate-950 flex font-sans selection:bg-brand-500/30">
    <button
      ...
      aria-label="Cerrar menu de ventas"
      ...
      <div class="mt-auto p-6 border-t border-slate-800">
         <div class="bg-slate-950/50 rounded-[2rem] p-4 border border-slate-800 flex items-center gap-4 group cursor-pointer hover:border-brand-500/30 transition-all">
            ...
            <div class="flex-1 min-w-0">
             ...
             <button @click="logout" type="button" aria-label="Cerrar sesion" class="text-slate-600 hover:text-red-400 transition-colors p-1"><i class="bi bi-power"></i></button>
         </div>
      </div>
    </aside>

    <!-- Main Content -->
    <main class="flex-1 min-w-0 overflow-y-auto bg-[radial-gradient(circle_at_top_right,rgba(132,204,22,0.03),transparent_40%)]">
      <div class="sticky top-0 z-30 flex items-center gap-3 border-b border-slate-800 bg-slate-950/90 pl-4 pr-16 py-3 backdrop-blur-xl md:hidden">
        <button type="button" class="w-11 h-11 rounded-xl bg-slate-900 border border-slate-800 text-brand-400" aria-label="Abrir menu de ventas" @click="sidebarOpen = true">
          <i class="bi bi-list text-2xl"></i>
        </button>
        <div class="min-w-0">
          <p class="truncate text-sm font-black text-white">{{ currentNav.label }}</p>
          <p class="truncate text-[9px] font-bold uppercase tracking-widest text-slate-500">Ventas Easypoint</p>
        </div>
      </div>
      <div class="p-4 md:p-8 lg:p-12 max-w-6xl mx-auto">
        <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-5 mb-8 md:mb-12">
           <div>
           ...
           <button @click="showNewApp = true" class="w-full sm:w-auto justify-center bg-brand-500 hover:bg-brand-400 text-slate-900 font-black px-6 md:px-8 py-4 rounded-2xl text-xs uppercase tracking-widest shadow-xl shadow-brand-500/20 active:scale-95 transition-all flex items-center gap-3">
              <i class="bi bi-plus-lg text-lg"></i> Nuevo Prospecto
           </button>
        </header>

        <!-- Stats Overview -->
        <div v-if="section === 'dashboard'" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12 animate-fade-in">
           <div v-for="stat in stats" :key="stat.label" class="bg-slate-900 border border-slate-800 p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative overflow-hidden group hover:border-brand-500/30 transition-all">
    ...
    <Transition name="fade">
      <div v-if="showNewApp" class="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-6 bg-slate-950/80 backdrop-blur-md">
        <div class="bg-slate-900 border border-slate-800 rounded-[2rem] md:rounded-[3rem] p-5 md:p-10 max-w-xl w-full max-h-[calc(100vh-1.5rem)] overflow-y-auto shadow-2xl relative animate-scale-in">
