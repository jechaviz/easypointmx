import { pbApi } from './_shared.mjs';

async function seed() {
  const p1 = await (await fetch(pbApi('/api/collections/points/records'), {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name:'Punto CDMX Centro',address:'Av. Juárez 14',active:true})
  })).json();
  
  const p2 = await (await fetch(pbApi('/api/collections/points/records'), {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name:'Locker Roma Exprés',address:'Obregón 201',active:true})
  })).json();

  await fetch(pbApi('/api/collections/shipments/records'), {
    method:'POST',
    headers:{'Content-Type':'application/json'},
